import React, { useState, KeyboardEvent, useRef, ChangeEvent, ChangeEventHandler, useEffect } from 'react'
import './style.css';
...
export default function Authentication() {
  ...
  //    event handler: 이메일 인풋 키 다운 이벤트 처리함수   //
  ...
  const SignUpCard = () => {
    ...
    //    state: 닉네임 확인 에러 메세지 상태    //
    const [nicknameErrorMessage, setNicknameErrorMessage] = useState<string>('');
    //    state: 휴대폰 확인 에러 메세지 상태    //
    ...
    //    function: sign up response 처리 함수오픈 함수    //
    ...
    //    event handler: 휴대폰번호 확인 변경 이벤트 처리    //
    ...
    //    event handler: 상세 주소 확인 변경 이벤트 처리    //
    ...
    const onAddressButotonClickHandler = () => {
      open( {onComplete});
    }
    ...
    const onSignUpButtonClickHandler = () => {
      ...
      if(!hasNickname) {
        setNicknameError(true);
        setNicknameErrorMessage('닉네임을 입력해주세.');
      }
      const telNumberPatter = /^[0-9]{11,13}$/;
      const isTelNumberPatter = telNumberPatter.test(telNumber);
      ...
      if(!hasNickname || !isTelNumberPatter || !agreedPersonal) return;
      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            {page === 1 && (
              <>
              <InputBox ref={emailRef} label='이메일 주소*' type='text' placeholder='이메일주소를 입력해주세요' value={email} onChange={onEmailChangeHandler} error={isEmailError} message={emailErrorMessage} onKeyDown={onEmailKeyDownHandler}/>
            ...
            {page === 2 && (
              ...
              <InputBox ref={telNumberRef} label='휴대 번호*' type='text' placeholder='휴대폰 번호를 입력해주세요' value={telNumber} onChange={onTelNumberChangeHandler} error={isTelNumberError} message={telNumberErrorMessage} onKeyDown={onTelNumberKeyDownHandler}/>
              <InputBox ref={addressRef} label='주소*' type='text' placeholder='주소를 입력해주세요' value={address} onChange={onAddressChangeHandler} error={isAddressError} message={addressErrorMessage} icon='expend-right-light-icon' onButtonClick={onAddressButotonClickHandler} onKeyDown={onAddressKeyDownHandler}/>
        ...
        <div className='auth-jumbotron-box'>
          <div className='auth-jumbotron-contents'>
            ...
            <div className='auth-jumbotron-text-box'>
              ...
              <div className='auth-jumbotron-text'>{'Kans Board 입니다.'}</div>
            </div>
          </div>
        </div>
